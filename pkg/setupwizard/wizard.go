...
outer:
	for {
		...
		if err := appendProfilePrimer(result); err != nil {
			result.EnableNotes = append(result.EnableNotes, fmt.Sprintf("profile hint (%s)", err))
		}
...
func writeServiceFile(path, workdir, logPath string, args []string, userUnit bool) error {
	...
	// #nosec G304 -- logPath is resolved by the installer into /var/log or the current user's state directory.
	...
	quotedWorkdir, err := quoteSystemdArgument(workdir)
	if err != nil {
		return err
	}
	quotedLogPath, err := quoteSystemdArgument(logPath)
	if err != nil {
		return err
	}
...
[Install]
WantedBy=%s
`, extractPort(args), quotedWorkdir, strings.Join(quotedArgs, " "), quotedLogPath, quotedLogPath, wantedBy)

	// #nosec G304 -- path is restricted by resolveServiceDestination to a systemd configuration directory.
	if err := os.WriteFile(path, []byte(content), 0o600); err != nil {
		return fmt.Errorf("write service file: %w", err)
	}
	if err := os.Chmod(path, 0o600); err != nil {
		return fmt.Errorf("secure service file: %w", err)
	}
	return nil
}

func quoteSystemdArgument(argument string) (string, error) {
	if strings.ContainsAny(argument, "\x00\r\n") {
		return "", errors.New("systemd argument contains control characters")
	}
	...
	go func() {
		...
		for _, cmdLine := range commands {
			...
			if parts[0] != "systemctl" {
				results <- commandResult{Message: cmdLine, Err: errors.New("unsupported service command")}
				continue
			}
			// #nosec G204 -- the executable is validated above and commands are generated by systemctlCommands.
...
func appendProfilePrimer(res Result) error {
	home, err := os.UserHomeDir()
	if err != nil {
		return err
	}
	...
	lines := []string{
		"Chicha service: " + res.ServiceName,
		"reload:  " + prefix + " daemon-reload",
		"restart: " + prefix + " restart " + res.ServiceName,
		"stop:    " + prefix + " stop " + res.ServiceName,
		"edit:    " + edit,
		"logs:    " + journal + " " + res.ServiceName + " -f (or tail -f " + res.LogPath + ")",
	}
	var block strings.Builder
	block.WriteString("\n# chicha-isotope-map service hint\nif [ -t 1 ]; then\n")
	for _, line := range lines {
		quoted, quoteErr := quoteShellArgument(line)
		if quoteErr != nil {
			return quoteErr
		}
		block.WriteString("  echo ")
		block.WriteString(quoted)
		block.WriteByte('\n')
	}
	block.WriteString("fi\n")

	// #nosec G304 -- profilePath is the fixed .profile file beneath os.UserHomeDir.
	...
	// #nosec G304 -- profilePath is the fixed .profile file beneath os.UserHomeDir.
	f, err := os.OpenFile(profilePath, os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0o600)
	if err != nil {
		return err
	}
...
func createUpdateScript(res Result) error {
	...
	serviceName, err := quoteShellArgument(res.ServiceName)
	if err != nil {
		return err
	}
	logPath, err := quoteShellArgument(res.LogPath)
	if err != nil {
		return err
	}
	quotedBinaryPath, err := quoteShellArgument(binaryPath)
	if err != nil {
		return err
	}
	controlArgs := "(systemctl)"
	if res.UserUnit {
		controlArgs = "(systemctl --user)"
	}

	script := fmt.Sprintf("#!/bin/bash\nset -euo pipefail\nSVC=%s\nLOG=%s\nBIN=%s\nCTL=%s\n\"${CTL[@]}\" stop \"$SVC\"\ncurl --fail --location --output \"$BIN\" https://github.com/matveynator/chicha-isotope-map/releases/download/latest/chicha-isotope-map_linux_amd64\nchmod 0750 \"$BIN\"\n\"$BIN\" --version\n\"${CTL[@]}\" start \"$SVC\"\ntail -f -- \"$LOG\"\n", serviceName, logPath, quotedBinaryPath, controlArgs)

	// #nosec G306 -- this root-owned helper must be executable; all embedded values are shell-quoted above.
	if err := os.WriteFile("/usr/local/bin/chicha-update", []byte(script), 0o750); err != nil {
		return fmt.Errorf("write updater: %w", err)
	}
	return nil
}

func quoteShellArgument(argument string) (string, error) {
	if strings.ContainsAny(argument, "\x00\r\n") {
		return "", errors.New("shell argument contains control characters")
	}
	return "'" + strings.ReplaceAll(argument, "'", "'\\''") + "'", nil
}
...
func tailLogs(ctx context.Context, out io.Writer, theme colorTheme, logPath string) {
	...
	// #nosec G204 -- tail is a fixed executable and -- prevents the local log path from becoming an option.
